...
/// to `credentials.json` so `orlop login` writes both in one place.
pub fn default_config_path() -> anyhow::Result<PathBuf> {
    Ok(crate::util::home_dir()?.join(".config/orlop/config.yaml"))
}

/// YAML body that `orlop login` seeds when no config exists yet. Mountpoint
...
pub fn default_config_yaml(home: &Path) -> String {
    ...
    format!(
        r#"# Auto-generated by `orlop login`. Edit to customize.
