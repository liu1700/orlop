...
const (
	...
	ChunkMax = 16 * 1024 * 1024
)

// Chunk is one content-defined slice with its BLAKE3 hash and absolute
...
// ChunkFile streams a file through FastCDC v2020 and returns each chunk's
// bytes + hash. Empty files produce a single zero-length chunk.
...
// ChunkReader is the streaming counterpart to ChunkFile.
// Reads all data into memory then chunks it — matches the Rust side which
// does the same (buffer model).
...
// chunkBytes applies FastCDC v2020 (Normalization Level 1, seed=0) to data
// using the same GEAR/GEAR_LS tables as the Rust `fastcdc` 3.x crate.
// See: https://ieeexplore.ieee.org/document/9055082
func chunkBytes(data []byte) []Chunk {
	if len(data) == 0 {
		var emptyHash [HashLen]byte
		empty := blake3.Sum256(nil)
		copy(emptyHash[:], empty[:])
		return []Chunk{{Hash: emptyHash, Offset: 0, Len: 0, Bytes: nil}}
	}

	// avg_size = 4 MiB = 2^22, bits = 22, normalization level 1
	// mask_s = MASKS[bits+1] = MASKS[23]
	// mask_l = MASKS[bits-1] = MASKS[21]
	const (
		maskS   uint64 = 0x0000d93777537000
		maskL   uint64 = 0x0000d91767537000
		maskSLS uint64 = maskS << 1
		maskLLS uint64 = maskL << 1
	)
	...
	for len(remaining) > 0 {
		length := v2020Cut(remaining, ChunkMin, ChunkAvg, ChunkMax, maskS, maskL, maskSLS, maskLLS)
		buf := make([]byte, length)
		copy(buf, remaining[:length])
...
// v2020Cut finds the cut point for the next chunk using the FastCDC 2020
// algorithm. Returns the number of bytes in the next chunk (1 ≤ result ≤ max).
// This is a direct port of the Rust fastcdc 3.x v2020::cut_gear function.
func v2020Cut(src []byte, minSize, avgSize, maxSize int, maskS, maskL, maskSLS, maskLLS uint64) int {
	n := len(src)
	if n <= minSize {
		return n
	}
	...
	// Small-mask phase: index in [minSize/2, center/2)
	for index < center/2 {
		a := index * 2
	...
	// Large-mask phase: index in [center/2, n/2)
	for index < n/2 {
		a := index * 2
		hash = (hash << 2) + gearLS[src[a]]
		if (hash & maskLLS) == 0 {
...
// gear is the FastCDC v2020 GEAR table (256 uint64 values).
// Computed as the high 8 bytes of the MD5 digest of [0..255].
// Source: https://github.com/nlfiedler/fastcdc-rs crate v3.x, src/v2020/mod.rs
var gear = [256]uint64{
...
// gearLS is the FastCDC v2020 GEAR_LS table: gear with every value
// left-shifted by 1 bit (the odd-position lane of the 2-bytes-per-iteration
// gear hash in v2020Cut). Derived from gear at init so the two tables cannot
// drift; must stay bit-identical to the Rust `fastcdc` 3.x crate's GEAR_LS
// (src/v2020/mod.rs) or Go and Rust chunk boundaries diverge.
var gearLS = func() [256]uint64 {
	var t [256]uint64
	for i, v := range gear {
		t[i] = v << 1
	}
