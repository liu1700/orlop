...
func (s *Service) Resize(
	...
	alloc, err := s.store.GetAllocation(ctx, toUUID(allocationID))
	...
	if delta > 0 {
		if err := s.store.ReserveCapacityForGrowth(ctx, server.ID, delta); err != nil {
			if errors.Is(err, storage.ErrNotFound) {
				return Allocation{}, ErrNoCapacity
			}
			return Allocation{}, fmt.Errorf("allocations: reserve grow delta: %w", err)
		}
	} else if err := s.store.ReleaseCapacity(ctx, server.ID, -delta); err != nil {
	...
	row, err := s.store.UpdateAllocationSize(ctx, toUUID(allocationID), toUUID(userID), newSizeBytes)
	if err != nil {
...
func (s *Service) compensatePoolDelta(ctx context.Context, serverID uuid.UUID, delta int64, reason string, cause error) {
	cctx, cancel := context.WithTimeout(context.WithoutCancel(ctx), 5*time.Second)
	defer cancel()
	var err error
	if delta > 0 {
		err = s.store.ReleaseCapacity(cctx, serverID, delta)
	} else {
		...
		err = s.store.ReserveCapacityForGrowth(cctx, serverID, -delta)
	}
	if err != nil {
