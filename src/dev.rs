...
pub fn run_dev_up(opts: DevUpOpts) -> Result<()> {
    ...
    for sub in ["orlop-data/objects", "orlop-data/tenants", "orlop-secrets", "cert"] {
...
/// teardown was clean. A signal-initiated shutdown is success;
...
/// needs no separate `doctor` step. Failures carry the same actionable
...
struct DevStackStatus {
    ...
    /// The `dev up` supervisor. `None` only for a state file written by an
    /// older orlop that never recorded it, in which case health falls back to
    /// the children.
...
/// stack that died uncleanly is reported as DEAD rather than a false UP.
pub fn run_status(json: bool) -> Result<()> {
    ...
    let dev_status = dev.map(|d| {
        ...
        // supervisor_pid == 0 means an older state file that never recorded it.
